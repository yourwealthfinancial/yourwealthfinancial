...
import { writeFileSync as writeFileAtomicSync2 } from "atomically";
...
`;
      ...
      if (error.name === "SyntaxError") {
        writeFileAtomicSync2(this.path, "");
        return {};
      }
      throw error;
    }
  ...
  set all(val) {
    try {
      ...
      writeFileAtomicSync2(this.path, JSON.stringify(val, null, "	"));
    } catch (error) {
      if (error.code === "EACCES") {
        error.message = `${error.message}
${permissionError}
...
var Fixture = class {
  ...
  async create() {
    if (!this.directory) {
      ...
      this.directory.path = await fs3.realpath(this.directory.path);
    }
